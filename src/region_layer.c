...
void make_region_layer(layer *l, int batch, int w, int h, int n, int classes, int coords)
{
    l->type = REGION;
...
void delta_region_class(float *output, float *delta, int index,
                        int class, int classes, tree *hier, float scale,
                        int stride, float *avg_cat)
{
    if (hier) {
        float pred = 1;
        while (class >= 0) {
    ...
    if (l->softmax_tree) {
        ...
        for (int i = 0; i < l->softmax_tree->groups; ++i) {
            ...
            softmax_cpu(net->input + count, group_size, l->batch, l->inputs,
                        l->n * l->w * l->h, 1, l->n * l->w * l->h, l->temperature, l->output + count);
            count += group_size;
        }
    }else if (l->softmax) {
        int index = entry_index(l, 0, 0, l->coords + !l->background);
        softmax_cpu(net->input + index, l->classes + l->background,
...
void forward_region_layer_gpu(layer *l)
{
    ...
    if (l->softmax_tree) {
        ...
        softmax_tree(net->input_gpu + index, l->w * l->h, l->batch * l->n, l->inputs / l->n, 1,
                     l->output_gpu + index, *l->softmax_tree);
        /*
        // TIMING CODE
        int number = 1000;
        int count = 0;
        ...
        */
        ...
        int index = entry_index(l, 0, 0, l->coords + !l->background);
        //printf("%d\n", index);
        softmax_gpu(net->input_gpu + index, l->classes + l->background, l->batch * l->n,
                    l->inputs / l->n, l->w * l->h, 1, l->w * l->h, 1, l->output_gpu + index);
    }
