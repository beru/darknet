...
void backward_convolutional_layer_gpu(convolutional_layer *l, network *net)
{
    ...
    if (net->delta_gpu) {
        ...
        if (l->xnor) {
            gradient_array_ongpu(original_input, l->batch*l->c*l->h*l->w, HARDTAN, net->delta_gpu);
        }
    }
    ...
    for (int i = 0; i < l->batch; ++i) {
        ...
        if (net->delta_gpu) {
            ...
            if (l->xnor) {
                gradient_array_ongpu(original_input + i * l->c * l->h * l->w, l->c * l->h * l->w, HARDTAN, net->delta_gpu + i * l->c * l->h * l->w);
            }
        }
    }
#endif
...
void adam_update_gpu(float *w, float *d, float *m, float *v, float B1, float B2,
                     float eps, float decay, float rate, int n, int batch, int t)
{
    scal_ongpu(n, B1, m, 1);
    scal_ongpu(n, B2, v, 1);
