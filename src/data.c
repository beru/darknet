...
{
    char **random_paths = xplat_malloc(n, sizeof(char*));
#ifdef THREAD
...
{
    char **replace_paths = xplat_malloc(n, sizeof(char*));
    for (int i = 0; i < n; ++i) {
        char replaced[4096];
...
matrix load_image_paths_gray(char **paths, int n, int w, int h)
{
    matrix X;
    X.rows = n;
    X.vals = xplat_malloc(X.rows, sizeof(float*));
    X.cols = 0;
...
{
    box_label *boxes = xplat_malloc(1, sizeof(box_label));
    FILE *file = fopen(filename, "r");
    if (!file) {
        file_error(filename);
    }
...
void fill_hierarchy(float *truth, int k, tree *hierarchy)
{
    ...
    int count = 0;
    for (int j = 0; j < hierarchy->groups; ++j) {
        //printf("%d\n", count);
        int mask = 1;
        for (int i = 0; i < hierarchy->group_size[j]; ++i) {
            if (truth[count + i]) {
                mask = 0;
                break;
            }
...
void fill_bg_mask(image m)
{
    ...
    for (int k = 0; k < m.c - 1; ++k) {
        for (int i = 0; i < m.w * m.h; ++i) {
            if (m.data[index + i] && m.data[k * m.w * m.h + i]) {
                m.data[index + i] = 0; 
            }
        }
    }
...
data load_data_seg(int n, char **paths, int m, int w, int h,
                   int classes, int min, int max, float angle, float aspect,
                   float hue, float saturation, float exposure)
{
    ...
    d.X.rows = n;
    d.X.vals = xplat_malloc(d.X.rows, sizeof(float*));
    d.X.cols = h*w*3;


    d.y.rows = n;
    d.y.cols = h * w * (classes + 1);
    d.y.vals = xplat_malloc(d.X.rows, sizeof(float*));
...
data load_data_compare(int n, char **paths, int m, int classes, int w, int h)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        d.X.vals[i] = xplat_malloc(d.X.cols, sizeof(float));
...
data load_data_detection(int n, char **paths, int m, int w, int h,
                         int boxes, int classes, float jitter,
                         float hue, float saturation, float exposure)
{
    char **random_paths = get_random_paths(paths, n, m);
    data d = {0};
...
void *load_data_impl(struct load_args *pa)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA) {
        ...
        *a.d = load_data_regression(a.paths, a.n, a.m,
                                    a.min, a.max, a.size, a.angle, a.aspect,
                                    a.hue, a.saturation, a.exposure);
    }else if (a.type == CLASSIFICATION_DATA) {
        *a.d = load_data_augment(a.paths, a.n, a.m, a.labels, a.classes, a.hierarchy,
                                 a.min, a.max, a.size, a.angle, a.aspect,
                                 a.hue, a.saturation, a.exposure, a.center);
    }else if (a.type == SUPER_DATA) {
        *a.d = load_data_super(a.paths, a.n, a.m, a.w, a.h, a.scale);
    }else if (a.type == WRITING_DATA) {
        *a.d = load_data_writing(a.paths, a.n, a.m, a.w, a.h, a.out_w, a.out_h);
    }else if (a.type == SEGMENTATION_DATA) {
        *a.d = load_data_seg(a.n, a.paths, a.m, a.w, a.h, a.classes,
                             a.min, a.max, a.angle, a.aspect,
                             a.hue, a.saturation, a.exposure);
        ...
        *a.d = load_data_tag(a.paths, a.n, a.m, a.classes,
                             a.min, a.max, a.size, a.angle, a.aspect,
                             a.hue, a.saturation, a.exposure);
    }
    return 0;
...
{
    pthread_t thread;
    struct load_args *ptr = xplat_malloc(1, sizeof(struct load_args));
    *ptr = args;
...
/*
   ...
   {
   data d = {0};
   d.indexes = xplat_malloc(n, sizeof(int));
...
data load_data_super(char **paths, int n, int m, int w, int h, int scale)
{
    ...
    d.X.rows = n;
    d.X.vals = xplat_malloc(n, sizeof(float*));
    d.X.cols = w*h*3;

    d.y.rows = n;
    d.y.vals = xplat_malloc(n, sizeof(float*));
...
data load_data_regression(char **paths, int n, int m,
                          int min, int max, int size, float angle, float aspect,
                          float hue, float saturation, float exposure)
{
    if (m) paths = get_random_paths(paths, n, m);
    data d = {0};
...
data load_data_augment(char **paths, int n, int m, char **labels, int k, tree *hierarchy,
                       int min, int max, int size, float angle, float aspect,
                       float hue, float saturation, float exposure, int center)
{
    if (m) paths = get_random_paths(paths, n, m);
...
data load_data_tag(char **paths, int n, int m, int k,
                   int min, int max, int size, float angle, float aspect,
                   float hue, float saturation, float exposure)
{
    if (m) paths = get_random_paths(paths, n, m);
...
matrix concat_matrix(matrix m1, matrix m2)
{
    ...
    m.vals = xplat_malloc(m1.rows + m2.rows, sizeof(float*));
    int count = 0;
...
data get_random_data(data d, int num)
{
    ...
    r.X.vals = xplat_malloc(num, sizeof(float *));
    r.y.vals = xplat_malloc(num, sizeof(float *));
...
{
    data *split = xplat_malloc(2, sizeof(data));
    ...
    train.X.vals = xplat_malloc(train.X.rows, sizeof(float*));
    test.X.vals = xplat_malloc(test.X.rows, sizeof(float*));
    train.y.vals = xplat_malloc(train.y.rows, sizeof(float*));
    test.y.vals = xplat_malloc(test.y.rows, sizeof(float*));
